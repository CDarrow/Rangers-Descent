...
void read_flying_controls( object * obj )
{
	...
	if (Players[Player_num].spec_flags & PLAYER_FLAGS_SPECTATING)			// jinx 08-07-13 spec
	{
		...
		HUD_init_message(HM_MULTI, "spec_booster: %i", spec_booster);
		...
		{
			spec_speed_z = Controls.forward_thrust_time + (Controls.forward_thrust_time * spec_booster / 100);
			spec_speed_x = Controls.sideways_thrust_time + (Controls.sideways_thrust_time * spec_booster / 100);
			spec_speed_y = Controls.vertical_thrust_time + (Controls.vertical_thrust_time * spec_booster / 100);
		}
	}
