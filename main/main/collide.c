...
void drop_player_eggs(object *playerobj)
{
	if ((playerobj->type == OBJ_PLAYER) || (playerobj->type == OBJ_GHOST) || (playerobj->type == OBJ_CAMERA)) 	// jinx 01-26-13 spec
	{
		...
		if (Players[pnum].spec_flags & PLAYER_FLAGS_SPECTATING)		// jinx 01-26-13 spec
			if (Players[pnum].secondary_ammo[CONCUSSION_INDEX] >= 7 - Difficulty_level)
				Players[pnum].secondary_ammo[CONCUSSION_INDEX] -= 7 - Difficulty_level;
		drop_missile_1_or_4(playerobj,CONCUSSION_INDEX);
		if ((Players[pnum].secondary_ammo[CONCUSSION_INDEX] < 0) || (Players[pnum].secondary_ammo[CONCUSSION_INDEX] > 10)) Players[pnum].secondary_ammo[CONCUSSION_INDEX] = 7 - Difficulty_level;		// jinx 08-07-13 spec
			drop_missile_1_or_4(playerobj,CONCUSSION_INDEX);
...
void collide_player_and_powerup( object * player, object * powerup, vms_vector *collision_point ) {
	if (!Endlevel_sequence && !Player_is_dead && (player->id == Player_num ) && (!(Players[Player_num].spec_flags & PLAYER_FLAGS_SPECTATING)))		// jinx 01-25-13 spec
	{
		int powerup_used;
