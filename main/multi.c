...
int message_length[MULTI_MAX_TYPE+1] = {
   ...
   2,  // CLOAK
	...
	7, // MULTI_KILL_HOST
	5, // MULTI_KILL_CLIENT
	3, // MULTI_DO_SPEC_STATUS		// jinx 08-07-13 spec
	6 // MULTI_SPEC_FLAGS		// jinx 08-07-13 spec
};
...
multi_new_game(void)
{
	...
	for (i = 0; i < MAX_PLAYERS; i++)
	{
		...
		Players[i].spec_flags &= ~PLAYER_FLAGS_SPECTATING;		// jinx 08-07-13 spec
	}
...
void multi_send_message_end()
{
  ...
  key_toggle_repeat(0);
	...
	else if ((!strnicmp (Network_message,"/kill",5)) && (strnicmp (Network_message,"/killreactor",12)))
	{
		object * playerobj;
		playerobj = &Objects[Players[Player_num].objnum];
		...
		apply_damage_to_player(playerobj, playerobj, 1, 0);
...
void multi_disconnect_player(int pnum)
{
	...
	if (Players[pnum].connected == CONNECT_PLAYING)
	{
		if ((Players[Player_num].spec_flags & PLAYER_FLAGS_SPECTATING) && (!in_free) && (piggy_num == pnum))
			switch_between_piggies();	// jinx 08-07-13 spec
...
multi_process_data(char *buf, int len)
{
	...
	switch(type) 
	{
		...
		case MULTI_DO_SPEC_STATUS:
			multi_do_spec_status(buf);	break;		// jinx 01-28-13 spec
		case MULTI_SPEC_FLAGS:
			multi_do_spec_flags(buf); break;
		default:
			Int3();
	}
}
