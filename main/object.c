...
void render_object(object *obj)
{
	...
	for (p = 0; p <= N_players; p++)
		if ((obj == &Objects[Players[p].objnum]) && Players[p].spec_flags & PLAYER_FLAGS_SPECTATING)
			return;
	
	if (obj->type == OBJ_PLAYER)
		for (p = 0; p <= N_players; p++)
			if (obj == &Objects[Players[p].objnum])
				if (Players[p].spec_flags & PLAYER_FLAGS_SPECTATING)
					return;
